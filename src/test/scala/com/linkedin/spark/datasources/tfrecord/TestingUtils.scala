...
object TestingUtils extends Matchers {
    ...
    def ~==(right: Seq[Decimal], epsilon: Double = 1E-6): Boolean = {
      if (left.size === right.size) {
        (left zip right) forall {
          case (a, b) => a.toDouble === (b.toDouble +- epsilon)
        }
      } else false
    }
  }
  ...
  implicit class InternalRowWithAlmostEquals(val left: InternalRow) {
    ...
    private type valueCompare =
      (SpecializedGetters, SpecializedGetters, Int) => Boolean
    private def newValueCompare(dataType: DataType,
                                epsilon: Float = 1E-6F): valueCompare =
      dataType match {
        case NullType =>
          (left, right, ordinal) =>
        ...
        case DecimalType() =>
          (left, right, ordinal) =>
            left
              .getDecimal(
                ordinal,
                DecimalType.USER_DEFAULT.precision,
        ...
        case StringType =>
          (left, right, ordinal) =>
            left.getUTF8String(ordinal).getBytes === right
              .getUTF8String(ordinal)
              .getBytes
        ...
        case _ =>
          throw new RuntimeException(
            s"Cannot convert field to unsupported data type ${dataType}"
          )
      }
    ...
    /**
      ...
      */
    def ~==(right: InternalRow,
            schema: StructType,
            epsilon: Float = 1E-6F): Boolean = {
  ...
  implicit class RowWithAlmostEquals(val left: Row) {
    ...
    def ~==(right: Row, schema: StructType): Boolean = {
      if (schema != null && schema.fields.size == left.size && schema.fields.size == right.size) {
        val leftRowWithSchema =
          new GenericRowWithSchema(left.toSeq.toArray, schema)
        val rightRowWithSchema =
          new GenericRowWithSchema(right.toSeq.toArray, schema)
      ...
      if (left.size === right.size) {
        ...
        (leftDataTypes zip rightDataTypes).zipWithIndex.forall {
          ...
          case ((BinaryType, BinaryType), i) =>
            left.getAs[Array[Byte]](i).toSeq === right
              .getAs[Array[Byte]](i)
              .toSeq

          case ((ArrayType(FloatType, _), ArrayType(FloatType, _)), i) =>
            val leftArray =
              ArrayData.toArrayData(left.get(i)).toFloatArray().toSeq
            val rightArray =
              ArrayData.toArrayData(right.get(i)).toFloatArray().toSeq
            leftArray ~== (rightArray, epsilon)

          case ((ArrayType(DoubleType, _), ArrayType(DoubleType, _)), i) =>
            val leftArray =
              ArrayData.toArrayData(left.get(i)).toDoubleArray().toSeq
            val rightArray =
              ArrayData.toArrayData(right.get(i)).toDoubleArray().toSeq
            leftArray ~== (rightArray, epsilon)

          case ((ArrayType(BinaryType, _), ArrayType(BinaryType, _)), i) =>
            val leftArray = ArrayData
              .toArrayData(left.get(i))
              .toArray[Array[Byte]](BinaryType)
              .map(_.toSeq)
              .toSeq
            val rightArray = ArrayData
              .toArrayData(right.get(i))
              .toArray[Array[Byte]](BinaryType)
              .map(_.toSeq)
              .toSeq
          ...
          case (
              (
                ArrayType(ArrayType(FloatType, _), _),
                ArrayType(ArrayType(FloatType, _), _)
              ),
              i
              ) =>
            val leftArrays =
              ArrayData.toArrayData(left.get(i)).array.toSeq.map { arr =>
                ArrayData.toArrayData(arr).toFloatArray().toSeq
              }
            val rightArrays =
              ArrayData.toArrayData(right.get(i)).array.toSeq.map { arr =>
                ArrayData.toArrayData(arr).toFloatArray().toSeq
              }
          ...
          case (
              (
                ArrayType(ArrayType(DoubleType, _), _),
                ArrayType(ArrayType(DoubleType, _), _)
              ),
              i
              ) =>
          ...
          case (
              (
                ArrayType(ArrayType(BinaryType, _), _),
                ArrayType(ArrayType(BinaryType, _), _)
              ),
              i
              ) =>
